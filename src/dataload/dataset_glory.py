...
class TrainGraphDataset(TrainDataset):
    def __init__(self, filename, news_index, news_input, local_rank, cfg, neighbor_dict, news_graph, entity_neighbors, news_emb):
    ...
    def line_mapper(self, line, sum_num_news):
        ...
        unique_subset, _ = torch.unique(subset, sorted=True,
                                                     return_inverse=True)
        newemb = self.news_emb[unique_subset]
        ...
        return sub_news_graph, padded_maping_idx, candidate_input, candidate_entity, entity_mask, label, \
               sum_num_news + sub_news_graph.num_nodes, newemb
        ...
        while True:
            ...
            with open(self.filename) as f:
                for line in f:  # 对于每一个用户，调用一次line_mapper函数，产生该用户对应的子图，候选输入，候选实体等
                    ...
                    sub_newsgraph, padded_mapping_idx, candidate_input, candidate_entity, entity_mask, label, sum_num_news, newemb = self.line_mapper(
                        line, sum_num_news)
                    ...
                    labels.append(label)
                    newembs.append(torch.from_numpy(newemb))
                    ...
                    if len(clicked_graphs) == self.batch_size:
                        ...
                        yield batch, mappings, candidates, candidate_entity_list, entity_mask_list, newembs, labels
                        clicked_graphs, mappings, candidates, labels, candidate_entity_list, entity_mask_list, newembs = [], [], [], [], [], [], []
                        sum_num_news = 0
