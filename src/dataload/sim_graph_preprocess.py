...
from textgo import TextSim            # github开源项目，用于计算文本相似度
...
def read_raw_news(cfg, file_path, mode='train'):
    ...
    with open(file_path, 'r', encoding='utf-8') as f:
        for line in tqdm(f, total=num_line, desc=f"[{mode}]Processing raw news"):
            ...
            news_id, category, subcategory, title, abstract, url, t_entity_str, c_entity_str = split_line
            update_dict(target_dict=news_dict, key=news_id)  # 将新闻id添加到列表中，值用数字表示

            # Entity
            if c_entity_str:
                entity_ids = [obj["WikidataId"] for obj in json.loads(c_entity_str)]
                entity_values = [obj["SurfaceForms"] for obj in json.loads(c_entity_str)]           # 取出实体名
                [update_dict(target_dict=entity_dict, key=entity_id) for entity_id in entity_ids]  # 实体也添加到列表中
                [update_dict(target_dict=entity_ID, key=entity_id, value= entity_value) for entity_id, entity_value in zip(entity_ids, entity_values)]  # 实体也添加到列表中
            elif t_entity_str:
                entity_ids = [obj["WikidataId"] for obj in json.loads(t_entity_str)]
                entity_values = [obj["SurfaceForms"] for obj in json.loads(t_entity_str)]
                [update_dict(target_dict=entity_dict, key=entity_id) for entity_id in entity_ids]  # 实体也添加到列表中
                [update_dict(target_dict=entity_ID, key=entity_id, value=entity_value) for entity_id, entity_value in
                 zip(entity_ids, entity_values)]
            else:
        ...
        if mode == 'train':
            ...
            return news, news_dict, category_dict, subcategory_dict, entity_dict, word_dict, entity_ID
        else:  # val, test
            return news, news_dict, None, None, entity_dict, None, entity_ID
...
def prepare_preprocess_bin(cfg, mode):
    ...
    if cfg.reprocess is True:
        # Glove
        nltk_news, nltk_news_dict, category_dict, subcategory_dict, entity_dict, word_dict, entity_ID = read_raw_news(
            # 将信息全部用编号表示
        ...
        pickle.dump(entity_ID, open(Path(data_dir[mode]) / "entity_ID.bin", "wb"))
...
def prepare_news_graph(cfg, mode='train'):
    ...
    news = pickle.load(open(Path(data_dir[mode]) / "nltk_news.bin", "rb"))
    entity_ID = pickle.load(open(Path(data_dir[mode]) / "entity_ID.bin", "rb"))
