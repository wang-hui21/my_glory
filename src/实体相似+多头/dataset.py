...
class TrainGraphDataset(TrainDataset):
    def __init__(self, filename, news_index, news_input, local_rank, cfg, neighbor_dict, news_graph, entity_neighbors,
        ...
        self.news_emb = news_emb.to(local_rank, non_blocking=True)
    ...
    def line_mapper(self, line, sum_num_news):
        ...
        if not click_idx:  # 如果传入的subset是空列表，将其设置为包含一个元素0的列表，确保后面的代码可以正常运行
            subset = [0]

        subset = torch.tensor(subset, dtype=torch.long, device=device)  # 将subset转化为pytorch张量

        unique_subset, _ = torch.unique(subset, sorted=False, return_inverse=True)
        emb_idx = F.pad(unique_subset, (self.cfg.model.his_size - len(unique_subset), 0), "constant",
                        0)
        # mask = emb_idx != 0
        # newemb = self.news_emb[emb_idx,:].masked_fill(~mask.unsqueeze(-1), 0).view(self.cfg.model.his_size,
        #                                                                                        400)
        ...
        return sub_news_graph, padded_maping_idx, candidate_input, candidate_entity, entity_mask, label, \
               sum_num_news + sub_news_graph.num_nodes, emb_idx
        # newemb
        ...
        while True:
            ...
            with open(self.filename) as f:
                for line in f:  # 对于每一个用户，调用一次line_mapper函数，产生该用户对应的子图，候选输入，候选实体等
                    ...
                    sub_newsgraph, padded_mapping_idx, candidate_input, candidate_entity, entity_mask, label, sum_num_news, emb_idx = self.line_mapper(
                        line, sum_num_news)
                    ...
                    if len(clicked_graphs) == self.batch_size:
                        ...
                        # newembs = torch.stack(newembs)
                        emb_idxs = torch.stack(emb_idxs)

                        labels = torch.tensor(labels, dtype=torch.long)
                        yield batch, mappings, candidates, candidate_entity_list, entity_mask_list, emb_idxs, self.news_emb, labels
                        clicked_graphs, mappings, candidates, labels, candidate_entity_list, entity_mask_list, emb_idxs = [], [], [], [], [], [], []
                        sum_num_news = 0
...
class ValidGraphDataset(TrainGraphDataset):
    ...
    def line_mapper(self, line):
        ...
        unsubset, _ = torch.unique(subset, sorted=False, return_inverse=True)
        # emb_idx = F.pad(unique_subset, (self.cfg.model.his_size - len(unique_subset), 0), "constant",
        #                           0)

        # newemb = self.news_emb[unique_subset,:].view(1,len(unique_subset), 400)
        ...
        return batch, mapping_idx, candidate_input, candidate_entity, entity_mask, unsubset, labels
    ...
    def __iter__(self):
        for line in open(self.filename):
            if line.strip().split('\t')[3]:
                ...
                #     line)
                batch, mapping_idx, candidate_input, candidate_entity, entity_mask, unsubset, labels = self.line_mapper(
                    line)
            # yield batch, mapping_idx, clicked_entity, candidate_input, candidate_entity, entity_mask, labels
            yield batch, mapping_idx, candidate_input, candidate_entity, entity_mask, unsubset, self.news_emb, labels
