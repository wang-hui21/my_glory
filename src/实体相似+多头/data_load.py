...
def load_data(cfg, mode='train', model=None, local_rank=0):
    ...
    data_dir_time = {"train": cfg.dataset.train_dir_time, "val": cfg.dataset.val_dir_time, "test": cfg.dataset.test_dir_time}
    # ------------- load news.tsv-------------
    ...
    # graph = news_graph.to(local_rank, non_blocking=True)
    ...
    news_emb = torch.cat(stacked_news, dim=0)
    # glory_emb = model.module.global_news_encoder(news_emb, graph.edge_index)             # 需要确保news_emb维度为两维
